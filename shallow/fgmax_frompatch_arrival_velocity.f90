...
subroutine fgmax_frompatch(mx,my,meqn,mbc,maux,q,aux,dx,dy, &
    ...
    !----------------------------------------------------------------------
    ! Modified to use speed for flagging arrival time, rather than h or h+B
    ! Can only be used if FG_NUM_VAL > 1 so speeds are computed
    !----------------------------------------------------------------------
    ...
    if (FG_NUM_VAL == 1) then
        write(6,*) '*** Error, need FG_NUM_VAL > 1 to use speed to flag arrivals'
        stop
        endif
     ...
     67         format('++++ Doing update at t = ', e20.11,' min t_last = ', &
        ...
        if ((time >= fg%tstart_max) .and. (time <= fg%tend_max) .and. &
            ...
            do k=1,fg%npts
                ...
                ! modified to use speed rather than h or h+B
                s = fg_values(2,k)
                if (mask_fgrid(k) .and. &
                   (level >= fg%min_level_check) .and. &
                   (s > fg%arrival_tol) .and. &
